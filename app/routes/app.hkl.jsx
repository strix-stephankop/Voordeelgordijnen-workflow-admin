...
import { queryHkl } from "../supabase.server";
...
export const loader = async ({ request }) => {
  ...
  try {
    const { data, count } = await queryHkl({ from, to, search });
    return json({
      orders: data,
      total: count,
      page,
    ...
    console.error("Failed to load HKL orders:", e.message);
    return json({
      orders: [],
      total: 0,
      page,
...
function useSupabaseRealtime(supabaseUrl, supabaseKey, tables, onEvent) {
  ...
  useEffect(() => {
    ...
    let channel = client.channel("realtime-hkl");
...
function HklCard({ order }) {
  ...
  return (
    <Card padding="0">
      ...
      <Collapsible open={open} id={`hkl-${order.id}`}>
        <Divider />
...
async function generateDateRangePdf(orders, dateFrom, dateTo) {
  ...
  const c = {
    ...
    yellow: rgb(252 / 255, 162 / 255, 26 / 255),
  ...
  draw("HKL - Order Rapport", margin + 80, H - 30, { size: 16, bold: true, color: c.white });
  ...
  pages.forEach((p, i) => {
    ...
    p.drawText(safe("Voordeelgordijnen  |  HKL"), { x: margin, y: 28, size: 7, font, color: c.light });
  });
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `hkl-${dateFrom}-${dateTo}.pdf`;
  a.click();
  URL.revokeObjectURL(url);
}
    ...
    <Page fullWidth>
      ...
      <BlockStack gap="400">
        <InlineStack align="space-between" blockAlign="end">
          <BlockStack gap="100">
            <Text variant="headingXl" as="h1">HKL</Text>
            <Text variant="bodyMd" as="p" tone="subdued">
              Overzicht van alle HKL orders en hun XML data
            </Text>
          </BlockStack>
          <Button
            icon={CalendarIcon}
        ...
        {isLoading ? (
          ...
          <BlockStack gap="300">
            {orders.map((order) => (
              <HklCard key={order.id} order={order} />
            ))}
          </BlockStack>
        )}
